...
export default function Layout({ children }) {
  ...
  // Allow page-specific overrides (e.g., Contact and Dashboard want distinct header styles)
  const defaultNavText = scrolled ? "text-[#0A1628]" : "text-white";
  // REPLACE THIS BLOCK IN src/Layout.jsx
  const pageNavOverrides = {
    "/contact": {
      headerClass: "bg-white shadow-md py-3", // Force solid white background
      navText: "text-[#0A1628]",              // Force dark text
    },
    "/dashboard": {
      headerClass: "bg-white shadow-md py-3",
      navText: "text-[#0A1628]",
    },
    // Adding these now so your future Rooms pages don't break
    "/rooms": {
      headerClass: "bg-white shadow-md py-3",
      navText: "text-[#0A1628]",
    },
  ...
  const override = pageNavOverrides[location.pathname];
  const navTextColor = override ? override.navText : defaultNavText;
  const navHoverColor = "hover:text-[#C9A961]";

  const navLinks = [
    { name: "Home", path: createPageUrl("Home"), icon: Home },
    { name: "Rooms & Suites", path: createPageUrl("Rooms"), icon: Bed },
